...
with st.container():
    if st.session_state.descr =={}:
        ...
        if st.session_state.snn:
            ...
            if spike:
                st.session_state.dataset_generator = tf.data.Dataset.from_generator(lambda: sparse_data_generator_non_spiking(input_images=st.session_state.x_train,
                                                                                            input_labels=st.session_state.y_train,
                                                                                            batch_size=b_size,
                                                                                            nb_steps=n_steps, shuffle=True,
                                                                                            flatten=fl),
                                                    output_shapes=((None, timesteps, st.session_state.ip_shape[0], st.session_state.ip_shape[1], st.session_state.ip_shape[2]), (None, None)),
                                                    output_types=(tf.float64, tf.uint8))
                st.session_state.dataset_generator_test = tf.data.Dataset.from_generator(lambda: sparse_data_generator_non_spiking(input_images=st.session_state.x_test,
                                                                                            input_labels=st.session_state.y_test,
                                                                                            batch_size=b_size,
...
def run_model(model,loss,optimizer,epochs,batch_size):
    ...
    try:
        ...
        if st.session_state.snn:
            model_fit = model.fit(st.session_state.dataset_generator.repeat(count=batch_size), epochs=epochs,
                                    validation_data=st.session_state.dataset_generator_test.repeat(count=batch_size))
        else:
